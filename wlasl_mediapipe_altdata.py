...
# note: see if asl is affected by whether the left hand / right hand is used
  ...
  if list(master_dict.keys()).sort() == full_df.columns.to_list().sort():
     print("procesed")
  ...
  col_names = full_df.columns.to_list().sort()

  if active_masterdf == False:
     master_df = pd.DataFrame(columns=col_names)
     active_masterdf = True

  print("this is master_df pre-append")
  print(master_df)
  print(master_df.shape)

  master_df = master_df._append(master_dict, ignore_index=True)

  # AttributeError: 'DataFrame' object has no attribute 'append'

  print("this is master df appended")
  print(master_df)
  print(master_df.shape)
